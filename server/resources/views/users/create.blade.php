...
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="d-flex align-items-center mb-4 gap-3">
            ...
            <h3 class="mb-0">{{ __('users.add_user') }}</h3>
        </div>
                ...
                <form method="POST" action="{{ route('users.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="role" name="role" class="form-select @error('role') is-invalid @enderror" required>
                            <option value="user" {{ old('role') === 'user' ? 'selected' : '' }}>{{ __('users.role_user') }}</option>
                            <option value="superadmin" {{ old('role') === 'superadmin' ? 'selected' : '' }}>{{ __('users.role_superadmin') }}</option>
                        </select>
                        @error('role')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <button type="submit" class="btn btn-primary w-100">{{ __('users.create_user') }}</button>
                </form>
            </div>
        </div>
    </div>
